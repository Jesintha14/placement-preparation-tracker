...
import { APTITUDE_TOPICS_LIST, SAMPLE_QUESTIONS } from '../data/mockData';
...
export const AptitudePage: React.FC<AptitudePageProps> = ({ onTestCompleted }) => {
  ...
  const [previousAttempts, setPreviousAttempts] = useState<TestAttempt[]>(() => 
    DataService.getTestAttempts().filter(t => t.category === 'Aptitude')
  );
  ...
  const startTest = (topicSlug: string | null = null) => {
    ...
    let pool = SAMPLE_QUESTIONS.filter(q => q.category === 'Aptitude');
    if (topicSlug) {
      const filtered = pool.filter(q => q.topic.toLowerCase() === topicSlug.toLowerCase());
      if (filtered.length > 0) pool = filtered;
    }
    setQuestions(pool);
    setCurrentIndex(0);
    setSelectedAnswers({});
    setTimerSeconds(pool.length * 90); // 90 seconds per question
    setElapsedSeconds(0);
  ...
  const handleSubmitTest = () => {
    ...
    const topicTitle = selectedTopic
      ? APTITUDE_TOPICS_LIST.find(t => t.slug === selectedTopic)?.name || 'Quantitative Aptitude'
      : 'Comprehensive Aptitude Mock';
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            ...
            <span>Quantitative Aptitude Practice</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Master all 12 standard campus quantitative topics with timed MCQs, formula cheat sheets & solutions.
          </p>
        </div>
        ...
        {!testMode && (
          <button
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white font-bold text-xs rounded-xl shadow-sm transition"
          >
            <Play className="w-4 h-4 fill-white" />
            <span>Take Comprehensive Aptitude Mock</span>
          </button>
        )}
      </div>
      ...
      {testMode ? (
        <div className="bg-white rounded-3xl p-6 md:p-8 border border-slate-200/80 shadow-md space-y-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-100">
            <div>
              <span className="text-xs font-bold text-sky-600 uppercase tracking-wider">
                {selectedTopic ? APTITUDE_TOPICS_LIST.find(t => t.slug === selectedTopic)?.name : 'Full Aptitude Test'}
              </span>
          ...
          {!isSubmitted ? (
            ...
            <div className="space-y-6 animate-in fade-in">
              <div className="p-6 rounded-2xl bg-gradient-to-br from-sky-50 to-indigo-50 border border-sky-200 text-center space-y-3">
                ...
                <div className="flex justify-center gap-3 pt-2">
                  <button
                    ...
                    className="px-4 py-2 bg-sky-600 text-white text-xs font-bold rounded-xl hover:bg-sky-700 transition flex items-center gap-1.5"
                  >
              ...
              <div className="space-y-4">
                <h4 className="font-extrabold text-slate-900 text-sm">Detailed Answers & Step-by-Step Derivations</h4>
                {questions.map((q, idx) => {
                    ...
                    <div key={idx} className={`p-5 rounded-2xl border ${isCorrect ? 'bg-emerald-50/40 border-emerald-200' : 'bg-rose-50/30 border-rose-200'}`}>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs mb-3">
                        <div className="p-2 rounded-lg bg-white border border-slate-200">
                          <span className="text-slate-400 font-medium block">Your Answer:</span>
        ...
        /* 12 APTITUDE TOPICS GRID */
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {APTITUDE_TOPICS_LIST.map((topic) => (
              <div 
                key={topic.slug}
                className="bg-white p-5 rounded-2xl border border-slate-200/80 hover:border-sky-400 shadow-xs hover:shadow-md transition flex flex-col justify-between group"
              >
                <div>
                  <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="mt-2.5 p-2 rounded-lg bg-slate-50 border border-slate-100 text-[11px] font-mono text-slate-600">
                    <span className="text-[9px] font-bold uppercase tracking-wider text-slate-400 block mb-0.5">Quick Formula</span>
                    {topic.formula}
                  </div>
                </div>
          ...
          {previousAttempts.length > 0 && (
            <div className="bg-white rounded-3xl p-6 border border-slate-200/80 shadow-xs">
              <h3 className="text-sm font-extrabold text-slate-900 mb-3 flex items-center gap-2">
                ...
                <span>Previous Aptitude Test Attempts</span>
              </h3>
